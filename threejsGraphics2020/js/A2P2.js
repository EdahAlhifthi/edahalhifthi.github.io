...
function makeSolidsFnc() {
    ...
    function f(i, n) {
        ...
        let args = {color: "#00ffff", opacity: 0.8, transparent: true};
...
function initGui() {
    ...
    var objectTypes =  ['solids', 'Sphere', 'Icosahedron', 'Ring']
...
function updateObject(objectType) {
    ...
    switch (objectType) {
        ...
        case 'solids':  geom = solidsFnc();
                        break;

        case 'Ring':    geom = new THREE.RingGeometry(1);
                        break;
       
        
    }
    currentGeom = geom;
    solidsFnc = makeSolidsFnc(geom);
    scene.add(geom);
}
